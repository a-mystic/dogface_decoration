...
using namespace cv;
using namespace std;
String imagename="/storage/emulated/0/sunglass.png";
Mat glasses = imread(imagename, IMREAD_UNCHANGED);
int imagesize=0;

void overlayImage(const Mat &background, const Mat &foreground,
                  Mat &output, Point2i location)
{
   background.copyTo(output);       //이거를 주석처리하면 얼굴에 원이그려짐
...
Java_com_example_opencvpractice_MainActivity_detect(JNIEnv *env, jobject instance,
    ...
    float resizeRatio = resize(img_gray, img_resize, 400);
...
//                        (char *) "face %d found ", faces.size());
    ...
    for (int i = 0; i < faces.size(); i++) {
        ...
        int radius=cvRound((real_facesize_width+real_facesize_height)*0.15);
        Point overlaycenter((real_facesize_x)*0.9,(real_facesize_height+center.y)/3);
        ...
        overlayImage(img_input, resized_glasses, img_result, overlaycenter);
...
//               circle(img_result, eye_center, radius,
//                       Scalar(255, 0, 0));    //눈부분 표시하는거 (255,0,0)<-이부분이 원의색상
...
JNIEXPORT void JNICALL
Java_com_example_opencvpractice_MainActivity_setimage(JNIEnv *env, jobject thiz, jstring name,int size) {
    // TODO: implement setimage()
    imagename=env->GetStringUTFChars(name, 0);

    glasses=imread(imagename, IMREAD_UNCHANGED);

    //이미지별로 이미지 입력받아서 imagesize변수 수정예정
    imagesize=size;
}
